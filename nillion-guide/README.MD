...
curl -Ls "you can get url on official discord channel" > $HOME/.nillionapp/config/genesis.json
curl -Ls "you can get url on official discord channel" > $HOME/.nillionapp/config/addrbook.json
```
### Configure Seeds
```bash
# Setting up a seed peers

sed -i -e "s|^seeds *=.*|seeds = \"you can get url on official discord channel\"|" $HOME/.nillionapp/config/config.toml
```
